...
# ── Redirect VATHUB — MUTAT în pagina „Email-uri" (2026-09-12) ────────────────
# Redirectul mailurilor de autoritate fiscală spre vathub@cargotrack.ro nu mai
# pleacă din căsuțele personale: sursa e acum căsuța principală, iar lista de
# expeditori se administrează din „Email-uri → Redirect VATHUB"
# (`GET/PUT /emails/vathub/rules`, motor `app/services/vathub_inbox.py`).
# Configul e ACEEAȘI cheie `settings.vathub.redirect`; `source` decide cine îl
# folosește, deci calea de aici se poate reactiva fără deploy (source="personal").
# Endpoint-urile de configurare au fost scoase de aici ca să nu existe două
# locuri care scriu aceeași listă. `smtp-test` rămâne — verifică doar credențialele.
